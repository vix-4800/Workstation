...
This is not only a line-by-line bug review. Also evaluate whether the selected approach makes sense for the task. A technically working implementation may still be a poor fit if it adds unnecessary database schema or classes, duplicates existing logic, changes unrelated code, puts logic in the wrong layer or namespace, or solves the task in a more complex way than necessary. Report actionable design problems even when they do not break current behaviour.
...
- Read `references/architecture.md` when responsibilities, layering, controllers, services, domain logic, new classes, namespaces, abstractions, duplication, or maintainability are involved.
...
For every review, consider whether the change fits the task and the existing structure. Examine the implementation strategy in depth when the change introduces new structure or changes responsibilities.
...
- Do new classes live in the module and namespace that own their responsibility?
- Has business logic accumulated in a controller or another boundary where the project already has a suitable owner?
...
For a design finding, describe what becomes harder to change, understand, test, or operate. A foreseeable maintenance cost is enough; a current behaviour failure is not required.
