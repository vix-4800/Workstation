...
function __fish_codex_global_optspecs
	string join \n c/config= enable= disable= remote= remote-auth-token-env= strict-config i/image= m/model= oss local-provider= p/profile= s/sandbox= dangerously-bypass-hook-trust C/cd= worktree add-dir= dangerously-bypass-approvals-and-sandbox approve-for-me a/ask-for-approval= search no-alt-screen no-daemon h/help V/version
end
...
complete -c codex -n "__fish_codex_needs_command" -l no-daemon -d 'Run without the shared background server, even if it is already running'
...
complete -c codex -n "__fish_codex_needs_command" -a "tcp-tunnel" -d 'Internal: forward a local TCP socket through an HTTP/3 CONNECT proxy'
...
complete -c codex -n "__fish_codex_using_subcommand agents" -l no-daemon -d 'The agents overview requires a shared server; this option is rejected'
...
complete -c codex -n "__fish_codex_using_subcommand tcp-tunnel" -l proxy-url -d 'HTTPS origin of the HTTP/3 proxy' -r
complete -c codex -n "__fish_codex_using_subcommand tcp-tunnel" -l proxy-origins-file -d 'File containing one exact approved HTTPS proxy origin per line' -r -F
complete -c codex -n "__fish_codex_using_subcommand tcp-tunnel" -l target -d 'CONNECT target authority, including a nonzero port' -r
complete -c codex -n "__fish_codex_using_subcommand tcp-tunnel" -l listen-addr -r
complete -c codex -n "__fish_codex_using_subcommand tcp-tunnel" -s c -l config -d 'Override a configuration value that would otherwise be loaded from `~/.codex/config.toml`. Use a dotted path (`foo.bar.baz`) to override nested values. The `value` portion is parsed as TOML. If it fails to parse as TOML, the raw string is used as a literal' -r
complete -c codex -n "__fish_codex_using_subcommand tcp-tunnel" -l enable -d 'Enable a feature (repeatable). Equivalent to `-c features.<name>=true`' -r
complete -c codex -n "__fish_codex_using_subcommand tcp-tunnel" -l disable -d 'Disable a feature (repeatable). Equivalent to `-c features.<name>=false`' -r
...
complete -c codex -n "__fish_codex_using_subcommand mcp; and __fish_seen_subcommand_from login" -l no-browser -d 'Print the authorization URL and accept the callback URL without opening a browser'
...
complete -c codex -n "__fish_codex_using_subcommand app-server; and __fish_seen_subcommand_from daemon" -f -a "update" -d 'Update the daemon package (may interrupt running work)'
...
complete -c codex -n "__fish_codex_using_subcommand doctor" -l probe-filesystem-path -d 'Internal isolated filesystem probe; exits before loading configuration' -r -F
...
complete -c codex -n "__fish_codex_using_subcommand resume" -l no-daemon -d 'Run without the shared background server, even if it is already running'
...
complete -c codex -n "__fish_codex_using_subcommand fork" -l no-daemon -d 'Run without the shared background server, even if it is already running'
...
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents tcp-tunnel exec review login logout mcp plugin app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "agents" -d 'Browse all agent sessions on the shared local app-server daemon'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents tcp-tunnel exec review login logout mcp plugin app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "tcp-tunnel" -d 'Internal: forward a local TCP socket through an HTTP/3 CONNECT proxy'
